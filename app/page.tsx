...
export default function Home() {
  ...
  const [stampedTrips, setStampedTrips] = useState<string[]>([]);
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("family-adventure-stamps");
    if (saved) {
      try {
        const parsed = JSON.parse(saved) as string[];
        queueMicrotask(() => setStampedTrips(parsed));
      } catch {
        window.localStorage.removeItem("family-adventure-stamps");
      }
    }
    ...
    <main className="site-shell">
      ...
      <section className="next-adventure">
        ...
        <div className="vote-options">
          {[
            { place: "Lake Michigan", emoji: "⛵" },
            { place: "Smoky Mountains", emoji: "⛰️" },
            { place: "Backyard campout", emoji: "⛺" },
          ].map((option, index) => (
            <button
              key={option.place}
              onClick={() => setVotes((current) => current.map((vote, voteIndex) => voteIndex === index ? vote + 1 : vote))}
            >
      ...
      {importOpen && (
        <div className="dialog-backdrop" role="presentation" onMouseDown={() => setImportOpen(false)}>
          <div className="import-dialog" role="dialog" aria-modal="true" aria-labelledby="import-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <p className="import-lede">Choose photos and videos from the device in your hand, or prepare a Google Photos connection.</p>
              ...
              <>
                ...
                {importedMedia.length > 0 && (
                  <div className="import-preview">
                    <div><b>Fresh from your camera roll</b><button onClick={clearImportedMedia}>Clear</button></div>
                    ...
                    <p>These previews stay in this browser session. Private cloud storage can be connected when you’re ready to publish real family media.</p>
                  </div>
                )}
              </>
            ) : (
